...
func (d *Driver) Create() error {
	...
	case "ubuntu_14_04":
		_, err := drivers.RunSSHCommandFromDriver(d, "sudo apt-get -y update && DEBIAN_FRONTEND=noninteractive sudo apt-get install -yq curl")
		if err != nil {
			return err
		}
	case "centos_7":
		_, err := drivers.RunSSHCommandFromDriver(d, "sudo yum -y update && sudo yumt install -y curl")
		if err != nil {
			return err
		}
	}
